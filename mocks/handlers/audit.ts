...
export const auditHandlers = [
  ...
  http.get('*/api/audit/issues/:auditId', async ({ request }) => {
    ...
    const { page, nextCursor, hasMore, total } = paginateWithCursor(items, cursor, limit, (i) => i.id);
        ...
        async start(controller) {
          ...
          for (let i = 0; i < issues.length; i += 50) {
            ...
            const csv =
              chunk
                .map(
                  (iss) => `${iss.id},${iss.rule},${iss.severity},${iss.category},"${iss.message}",${iss.affectedUrls}`,
                )
                .join('\n') + '\n';
