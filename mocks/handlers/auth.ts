...
export const authHandlers = [
  ...
  http.post('*/api/auth/login', async ({ request }) => {
    ...
    if (!body.email || !body.password) {
      return HttpResponse.json({ error: 'VALIDATION_ERROR', message: '缺少 email 或 password' }, { status: 400 });
    }

    // 模擬錯誤帳號
    if (body.email === 'wrong@example.com') {
      return HttpResponse.json({ error: 'INVALID_CREDENTIALS', message: '帳號或密碼錯誤' }, { status: 401 });
    }
  ...
  http.post('*/api/auth/refresh', async ({ request }) => {
    ...
    if (!body.refreshToken) {
      return HttpResponse.json({ error: 'VALIDATION_ERROR', message: 'Missing refreshToken' }, { status: 400 });
    }

    // 模擬 refresh token 過期
    if (body.refreshToken === 'invalid' || body.refreshToken === 'expired_refresh') {
      return HttpResponse.json({ error: 'REFRESH_EXPIRED', message: 'Please re-login' }, { status: 401 });
    }
