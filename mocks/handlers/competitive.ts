...
export const competitiveHandlers = [
  ...
  http.get('*/api/competitive/compare', async ({ request }) => {
    ...
    // 計算排名
    for (const m of metrics) {
      const sorted = [...comparisons].sort((a, b) => b.metrics[m].value - a.metrics[m].value);
      sorted.forEach((c, i) => {
  ...
  http.get('*/api/competitive/keyword-gap', async ({ request }) => {
    ...
    if (gapType === 'missing') {
      ...
      items = items.filter((k) => {
        ...
        return Object.values(k.competitorPositions).some((p) => p !== null && p < k.myPosition!);
      });
    } else if (gapType === 'untapped') {
      items = items.filter(
        (k) => k.myPosition === null && Object.values(k.competitorPositions).filter((p) => p !== null).length >= 2,
      );
    }
    ...
    const { page, nextCursor, hasMore, total } = paginateWithCursor(items, cursor, limit, (k) => k.keyword);
  ...
  http.get('*/api/competitive/backlink-sources', async ({ request }) => {
    ...
    const matrix = domainList.map((domain) => {
      ...
      for (let i = 0; i < types.length; i++) {
        const val =
          i < types.length - 1 ? Math.round(remaining * faker.number.float({ min: 0.1, max: 0.4 })) : remaining;
