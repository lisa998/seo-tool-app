...
export const contentHandlers = [
    ...
    http.get('/api/content/search', async ({request}) => {
        ...
        if (sort.startsWith('socialShares')) {
            ...
            items = [...items].sort((a, b) =>
                desc
                    ? b.metrics.socialShares - a.metrics.socialShares
                    : a.metrics.socialShares - b.metrics.socialShares,
            )
        } else if (sort.startsWith('backlinks')) {
            const desc = sort.endsWith('desc')
            items = [...items].sort((a, b) =>
                desc
                    ? b.metrics.backlinks - a.metrics.backlinks
                    : a.metrics.backlinks - b.metrics.backlinks,
            )
    ...
    http.get('/api/content/trending', async ({request}) => {
        ...
        const sorted = [...items]
            .sort((a, b) => b.metrics.socialShares - a.metrics.socialShares)
            .slice(0, 20)
