...
export const siteExplorerHandlers = [
  ...
  http.get('*/api/site-explorer/overview', async ({ request }) => {
    ...
    const genSparkline = (base: number) =>
      Array.from({ length: 6 }, (_, i) => Math.round(base * (0.85 + i * 0.03 + Math.random() * 0.02)));
  ...
  http.get('*/api/site-explorer/backlinks', async ({ request }) => {
    ...
    if (search) {
      ...
      items = items.filter((b) => b.sourceUrl.toLowerCase().includes(q) || b.anchor.toLowerCase().includes(q));
    }
    ...
    // 分頁
    const { page, nextCursor, hasMore, total } = paginateWithCursor(items, cursor, limit, (b) => b.id);
  ...
  http.get('*/api/site-explorer/referring-domains', async ({ request }) => {
    ...
    const { page, nextCursor, hasMore, total } = paginateWithCursor(items, cursor, limit, (r) => r.id);
  ...
  http.get('*/api/site-explorer/broken-links', async ({ request }) => {
    ...
    const { page, nextCursor, hasMore, total } = paginateWithCursor(items, cursor, limit, (b) => b.targetUrl);
